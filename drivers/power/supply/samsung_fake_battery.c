...
static const struct power_supply_desc samsung_battery_desc = {
	.name = "battery",
...
static const struct power_supply_desc samsung_usb_desc = {
	.name = "usb",
...
static const struct power_supply_desc samsung_ac_desc = {
	.name = "ac",
...
static int samsung_fake_bat_probe(struct platform_device *pdev)
{
	...
	ret = power_supply_register(&pdev->dev,
				&samsung_battery_desc,
				&samsung_power_supplies[0]);
	if (ret) {
...
struct max77693_charger *chg_bat, *chg_ac, *chg_usb;
static int samsung_fake_bat_probe(struct platform_device *pdev)
{
	struct power_supply_config psy_cfg_bat = {}, psy_cfg_ac = {}, psy_cfg_usb = {};
	struct max77693_dev *max77693 = dev_get_drvdata(pdev->dev.parent);
	int ret;
	
	pr_err("%s: init\n", __func__);
	
	vbus_wake_lock = wakeup_source_create("vbus_present");
	...
	chg_bat = devm_kzalloc(&pdev->dev, sizeof(*chg_bat), GFP_KERNEL);
	if (!chg_bat)
		return -ENOMEM;

	platform_set_drvdata(pdev, chg_bat);
	chg_bat->dev = &pdev->dev;
	chg_bat->max77693 = max77693;

	/* set initial value */
	chg_bat->cable.connected = false;

	psy_cfg_usb.drv_data = chg_bat;

	chg_bat->charger = power_supply_register(&pdev->dev,
						&samsung_battery_desc,
						&psy_cfg_bat);
	if (IS_ERR(chg_bat->charger)) {
		pr_err("%s: failed: power supply chg_bat register\n", __func__);
		dev_err(&pdev->dev, "failed: power supply register\n");
		ret = PTR_ERR(chg_bat->charger);
		goto err;
	}

	
	chg_usb = devm_kzalloc(&pdev->dev, sizeof(*chg_usb), GFP_KERNEL);
	if (!chg_usb)
		return -ENOMEM;

	platform_set_drvdata(pdev, chg_usb);
	chg_usb->dev = &pdev->dev;
	chg_usb->max77693 = max77693;

	/* set initial value */
	chg_usb->cable.connected = false;

	psy_cfg_usb.drv_data = chg_usb;

	chg_usb->charger = power_supply_register(&pdev->dev,
						&samsung_usb_desc,
						&psy_cfg_usb);
	if (IS_ERR(chg_usb->charger)) {
		pr_err("%s: failed: power supply chg_usb register\n", __func__);
		dev_err(&pdev->dev, "failed: power supply register\n");
		ret = PTR_ERR(chg_usb->charger);
		goto err;
	}
	
	
	chg_ac = devm_kzalloc(&pdev->dev, sizeof(*chg_ac), GFP_KERNEL);
	if (!chg_ac)
		return -ENOMEM;

	platform_set_drvdata(pdev, chg_ac);
	chg_ac->dev = &pdev->dev;
	chg_ac->max77693 = max77693;

	/* set initial value */
	chg_ac->cable.connected = false;

	psy_cfg_ac.drv_data = chg_ac;

	chg_ac->charger = power_supply_register(&pdev->dev,
						&samsung_ac_desc,
						&psy_cfg_ac);
	if (IS_ERR(chg_ac->charger)) {
		pr_err("%s: failed: power supply chg_ac register\n", __func__);
		ret = PTR_ERR(chg_ac->charger);
		goto err;
	}
	
	pr_err("%s: registered successfully\n", __func__);
...
static int samsung_fake_bat_remove(struct platform_device *pdev)
{
	power_supply_unregister(chg_bat->charger);
	power_supply_unregister(chg_ac->charger);
	power_supply_unregister(chg_usb->charger);
	wakeup_source_destroy(vbus_wake_lock);

	return 0;
}

static const struct of_device_id init_device_ids[] = {
	{ .compatible = "stupid,samsung-fake-battery" },
	{},
};
/*
static const struct platform_device_id max77693_charger_id[] = {
	{ "samsung-fake-battery", 0, },
	{ }
};
...
//module_platform_driver(samsung_fake_bat_driver);

static int __init samsung_fake_bat_init(void)
{
	int ret = 0;
	ret = platform_driver_register(&samsung_fake_bat_driver);
	pr_err("%s: platform_driver_register returned %d\n", __func__, ret);
	return ret;
}
